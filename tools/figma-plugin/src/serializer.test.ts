...
describe('@designrail/figma-plugin serializer', () => {
  it('serializes a component set into a schema-valid fixture', () => {
    const { fileName, fixture, warnings } = serializeFixture(BUTTON_SET);

    expect(() => mockFigmaFixtureSchema.parse(fixture)).not.toThrow();
    expect(fileName).toBe('figma-input.button.primary.json');
    expect(fixture.exampleId).toBe('example.button.primary');
    expect(fixture.intentId).toBe('intent.button.primary');
  ...
  it('derives ids from slash-named components', () => {
    ...
    expect(fixture.exampleId).toMatch(/^example\.[a-z0-9-]+\.[a-z0-9-]+$/);
    expect(fixture.exampleId).toBe('example.icon-button.small-ghost');
    expect(fileName).toBe('figma-input.icon-button.small-ghost.json');
  });
  ...
  it('falls back with warnings when text, tokens, and variant groups are missing', () => {
    ...
    expect(() => mockFigmaFixtureSchema.parse(fixture)).not.toThrow();
    expect(fixture.exampleId).toBe('example.card.export');
