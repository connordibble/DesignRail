...
The import, mapping, and compliance tools are library-first and return structured JSON:

- `figma-import` returns a valid `ComponentIntent`
- `component-mapper` returns a valid `ComponentMapping`
- `compliance-agent` returns valid `ComplianceFinding[]`
